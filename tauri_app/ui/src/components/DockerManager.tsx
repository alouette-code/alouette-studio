...
import { Minus, Square as SquareIcon, X } from "lucide-react";
import { getCurrentWindow } from "@tauri-apps/api/window";
import WindowResizer from "./WindowResizer";
import brandIcon from "./logo_alouette.png";

const DockerIcon = ({ size = 14, className = "" }) => (
  <svg 
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: THEME.bgApp, color: THEME.textMain, fontFamily: "sans-serif", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <h2 style={{ color: THEME.textMain }}>Docker Management Coming Soon</h2>
        <p style={{ color: THEME.textMuted }}>This is the independent window for Docker.</p>
      </div>
    </div>
  );
}
